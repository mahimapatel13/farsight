...
int total() {
  ...
  for (var i = 0; i < history2.length; i++) {
    String cleanAmount = history2[i].amount.replaceAll(RegExp(r'[^0-9]'), '');
    if (cleanAmount.isEmpty) cleanAmount = '0';
    int amt = int.parse(cleanAmount);
...
int income() {
  ...
  for (var i = 0; i < history2.length; i++) {
    String cleanAmount = history2[i].amount.replaceAll(RegExp(r'[^0-9]'), '');
    if (cleanAmount.isEmpty) cleanAmount = '0';
    a.add(history2[i].IN == 'Income' ? int.parse(cleanAmount) : 0);
  }
  totals = a.reduce((value, element) => value + element);
  return totals;
}
...
int expenses() {
  ...
  for (var i = 0; i < history2.length; i++) {
    String cleanAmount = history2[i].amount.replaceAll(RegExp(r'[^0-9]'), '');
    if (cleanAmount.isEmpty) cleanAmount = '0';
    a.add(history2[i].IN == 'Income' ? 0 : int.parse(cleanAmount) * -1);
  }
  totals = a.reduce((value, element) => value + element);
  return totals;
}
